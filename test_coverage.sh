...
grcov . --binary-path ./target/debug/deps/ -s . -t html --branch --ignore-not-existing --ignore 'target/*' --ignore '/*' -o target/coverage/html/
# Also generate a lcov file for further processing
grcov . --binary-path ./target/debug/deps/ -s . -t lcov --branch --ignore-not-existing --ignore 'target/*' --ignore '/*' -o target/coverage/tests.lcov
