...
# If we don't remove these files, there might be a mixup between previous runs
# and the reported coverage is incosistent.
rm -f target/coverage/
...
grcov . --binary-path ./target/coverage-build/debug/deps/ -s . -t html --branch --ignore-not-existing --ignore 'target/*' --ignore '/*' --ignore '**/tests.rs' -o target/coverage/html/
# Also generate a lcov file for further processing
grcov . --binary-path ./target/coverage-build/debug/deps/ -s . -t lcov --branch --ignore-not-existing --ignore 'target/*' --ignore '/*' --ignore '**/tests.rs' -o target/coverage/tests.lcov
